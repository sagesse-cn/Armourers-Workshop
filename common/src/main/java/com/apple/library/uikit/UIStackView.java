...
public class UIStackView extends UIView {
    ...
    private void _applySize(CGRect bounds, List<CGRect> rects, Distribution distribution) {
        ...
        switch (distribution) {
            ...
            case FILL_EQUALLY: {
                var usableSize = bounds.width() - _spacing * (count - 1);
                var itemLeft = 0f;
            ...
            case FILL_PROPORTIONALLY: {
                var usableSize = bounds.width() - _spacing * (count - 1);
                var resolvedSize = _calcSize(rects);
                var contentWidth = Math.max(resolvedSize.width, 1);
                var itemLeft = 0f;
                for (var rect : rects) {
                    var pro = rect.width / contentWidth;
            ...
            case EQUAL_SPACING: {
                ...
                var resolvedSize = _calcSize(rects);
                var contentSize = Math.max(resolvedSize.width, 1);
                var spacing = (bounds.width - contentSize) / (count - 1);
                var itemLeft = 0f;
                for (var rect : rects) {
            ...
            case EQUAL_CENTERING: {
                var centerSize = bounds.width() / (count + 1);
                var itemLeft = 0f;
                for (var rect : rects) {
    ...
    private void _applyOffset(CGRect bounds, List<CGRect> rects, Alignment alignment) {
        ...
        switch (alignment) {
            case FILL: {
                var usableHeight = bounds.height();
                for (var rect : rects) {
                    rect.setY(0);
            ...
            case CENTER: {
                var usableHeight = bounds.height();
                for (var rect : rects) {
                    rect.setY((usableHeight - rect.height()) / 2);
                }
                break;
            }
            case FIRST_BASELINE:
